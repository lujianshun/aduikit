...
public class AddSpaceTextWatcher implements TextWatcher {
    ...
    private LabelDeleteEditText ldEdittext;
    /**
     * text最大长度限制
     */
    ...
    public AddSpaceTextWatcher(LabelDeleteEditText editText, int maxLenght) {
        this.ldEdittext = editText;
        this.maxLenght = maxLenght;
        if (editText == null) {
            new NullPointerException("editText is null");
        }
        ...
        editText.setFilters(new InputFilter[]{new InputFilter.LengthFilter(
          maxLenght)});
        editText.addTextWatcherListener(this);
    }
    ...
    public void afterTextChanged(Editable s) {
        if (isChanged) {
            if(editText != null){
                ...
                location = ldEdittext.getSelectionEnd();
            }
            int index = 0;
    ...
    private void updateContext(Editable editable,String values) {
        if (spaceType == SpaceType.IDCardNumberType) {
            ...
            if(editText != null){
                ...
                ldEdittext.setText(values);
                try {
                    ldEdittext.setSelection(location);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    public boolean setText(CharSequence charSequence) {
        if(ldEdittext != null && !TextUtils.isEmpty(charSequence) && computeSpaceCount(charSequence) <= maxLenght){
            isSetText = true;
            ldEdittext.removeTextWatcherListener(this);
            ldEdittext.setText(charSequence.toString());
            ldEdittext.addTextWatcherListener(this);
            return true;
        }
